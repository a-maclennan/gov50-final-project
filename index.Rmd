...
Since the filtered survey sample is relatively small (only 140 respondents), we utilize a bootstrap test to determine if it is alright to rely on this sample for results that would reflect a broader population. The bootstrap test randomly samples from the original survey sample with replacement in order to mimic the sampling process and help us determine whether a sample can represent a broader population. 
...
This study evaluated survey data of 140 Asian Americans provided by the nonprofit TARGET Ohio, which focuses on Asian American issues via data collection and voter outreach. This study found that there is a statistically significant positive relationship between *discrim* (whether a respondent indicated they or a family member had personally experienced racism) and *racism_issue* (whether that respondent ranked racism among their top three politically important issues). Specifically, this study's results estimate that, holding other factors constant, having personal experience with racism increases an Asian American's likelihood of ranking racism in their top three political issues by 34.8%. 

### Limitations and Possible Improvements

Many limitations of this project were found in the relative lack of demographic data in the dataset that would potentially have allowed for the controlling of additional variables. For example, the dataset did not include age, location, skin color, education level, occupation, family size, or many other potentially relevant demographic details. It would have been interesting to examine a potential link to colorism, for instance, if the survey had collected data on respondents' skin color. Darker-skinned Asian Americans live a different experience than lighter-skinned Asian Americans, and with skin color data, a study could examine whether those different experiences contribute to different political effects from experiencing racism. 

Another limitation of this project was that there was the number of respondents who ommitted responses to some or all of the questions of relevance on the survey, as well as respondents who listed more than one origin region (there was no easy way to include them, so they were filtered out of the dataset). Ideally, a study would be able to analyze responses from all respondents, so in a study with a larger sample, perhaps it would be possible to create a separate category for respondents from multiple origin regions.
